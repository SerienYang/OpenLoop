...
from openloop.mcp import build_callables, load_mcp_servers, tool_name
from openloop.mcp.config import MCPServerDef
...
def test_untrusted_workspace_mcp_ignored(tmp_path, monkeypatch):
    """#213: a cloned repo's `.openloop/mcp.json` must not load until trust."""
    monkeypatch.setenv("OPENLOOP_STATE_DIR", str(tmp_path / "state"))
    _write_json(
        tmp_path / "state" / "mcp.json",
        {
...
def test_schema_and_metadata():
    ...
    schema = fn.__openloop_schema__["function"]
