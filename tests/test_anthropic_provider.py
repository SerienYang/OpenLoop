...
from openloop.providers import AnthropicProvider, capabilities_for
from openloop.providers.anthropic_provider import (
    DEFAULT_MAX_TOKENS,
    convert_messages,
    convert_tools,
)
...
def test_basic_compatibility_mode_uses_explicit_base_url(monkeypatch):
    captured: dict = {}
...
def test_basic_compatibility_complete_and_stream_send_only_standard_payload():
    tools = [
        {
            "type": "function",
            "function": {
    ...
    _, converted = convert_messages(messages)
    expected = {
        "model": "qwen3.8-max",
        "messages": converted,
        "system": "system text",
    ...
    complete_client = _FakeClient(response=_text_response())
    AnthropicProvider(
    ...
    assert complete_client.kwargs == expected

    stream_client = _FakeClient(events=[])
    list(
        AnthropicProvider(
    ...
    assert stream_client.kwargs == {**expected, "stream": True}
...
def test_resolve_api_key_env_then_secrets(monkeypatch):
    from openloop.providers.anthropic_provider import resolve_api_key
...
def test_thinking_defaults_on_with_hidden_profile_override():
    ...
    from openloop.providers.anthropic_provider import DEFAULT_THINKING_BUDGET
    from openloop.providers.registry import build_provider_client
