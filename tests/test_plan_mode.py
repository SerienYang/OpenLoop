...
from openloop.permissions import Mode, PermissionEngine
...
from openloop.tools.plan import propose_plan_tool
...
def test_build_engine_plan_mode_wiring(tmp_path):
    ...
    engine = build_engine(
        agent=openloop_agent(), workspace=tmp_path, provider=_Stub(), mode=Mode.PLAN
    )
    try:
...
def test_build_engine_discuss_reminder_not_plan_contract(tmp_path):
    ...
    engine = build_engine(
        agent=openloop_agent(), workspace=tmp_path, provider=_Stub(), mode=Mode.DISCUSS
    )
    try:
