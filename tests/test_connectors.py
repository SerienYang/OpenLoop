...
from openloop.connectors import (
    ConnectorSettings,
    FakeAdapter,
    Gateway,
    MessageEvent,
...
def test_send_message_success(tmp_path):
    ...
    assert tool.__openloop_schema__["function"]["name"] == "send_message"
...
def test_load_settings_from_secretstore(tmp_path, monkeypatch):
    ...
    settings = __import__(
        "openloop.connectors.config", fromlist=["load_settings"]
...
class _StubProvider:
    ...
    def complete(self, **_kw):  # pragma: no cover - never invoked at build time
        from openloop.providers import AssistantTurn

        return AssistantTurn()

    def capabilities(self, _model):  # pragma: no cover
        from openloop.providers.base import ModelCapabilities

        return ModelCapabilities()

    def stream(self, **_kw):  # pragma: no cover
        from openloop.providers.base import StreamChunk

        yield StreamChunk(turn=self.complete())


def test_openloop_engine_exposes_connected_connector_tools(tmp_path):
    ...
    assert "send_message" not in empty.registry.names()
    assert "browser_read_url" in empty.registry.names()
    empty.executor.close()
    ...
    assert "send_message" in engine.registry.names()
    assert "browser_read_url" in engine.registry.names()
    assert "browser_open_url" in engine.registry.names()
    assert "browser_click" in engine.registry.names()
    assert "browser_type" in engine.registry.names()
    assert "github_search" not in engine.registry.names()

    # §36: browser READS (registry kind) are free; interactions still gate.
    assert engine.registry.get("browser_open_url").metadata.requires_approval is False
    assert engine.registry.get("browser_snapshot").metadata.requires_approval is False
    assert engine.registry.get("browser_click").metadata.requires_approval is True
    assert engine.registry.get("browser_type").metadata.requires_approval is True
    engine.permissions.allow_tool_for_session("browser_click")
    decision = engine.permissions.evaluate(
        "browser_click",
        {"target": "button"},
        engine.registry.get("browser_click").metadata,
    )
    assert decision.needs_user is True
    engine.executor.close()

    secrets.put("github:default", {"token": "ghp_test", "enabled": True})
    with_github = build_engine(
        agent=openloop_agent(),
        workspace=tmp_path,
        provider=_StubProvider(),
        secrets=secrets,
    )
    assert "github_search" in with_github.registry.names()
    # §36: github_search is a registry READ — free; the write sibling still gates.
    assert (
        with_github.registry.get("github_search").metadata.requires_approval is False
    )
    assert (
        with_github.registry.get("github_create_issue").metadata.requires_approval is True
    )
    with_github.executor.close()


# -- connector setup (descriptors / connect / disconnect / list) ---------------
def test_connector_list_descriptors(tmp_path):
    from openloop.connectors import connector_list
    ...
    # GitHub's local PAT path is request/response only.
    assert by_name["telegram"]["channels"] is True
    assert by_name["slack"]["channels"] is True
    assert by_name["github"]["two_way"] is False
    assert (
...
def test_connector_list_pre_connect_copy(tmp_path):
    ...
    from openloop.connectors import connector_list
    from openloop.connectors.catalog_copy import ACCESS
    from openloop.connectors.descriptors import list_descriptors
...
def test_reconnect_does_not_clobber_secret_or_allowlist(tmp_path):
    ...
    from openloop.connectors import connect_connector
...
def test_connect_validation_runs(tmp_path):
    from openloop.connectors import connect_connector
    from openloop.connectors.descriptors import ValidationResult, get_descriptor
...
def test_telegram_message_mapper():
    ...
    from openloop.connectors import telegram_message_to_event
...
def test_slack_event_mapper_and_loop_guard():
    from openloop.connectors import slack_event_to_event
...
def test_make_adapter():
    from openloop.connectors import SlackAdapter, TelegramAdapter, make_adapter
...
async def test_slack_resolves_and_caches_display_name():
    from openloop.connectors import SlackAdapter
...
async def test_slack_resolve_channel_name():
    ...
    class _Client:
        async def conversations_info(self, channel):
            ...
            return {"channel": {"id": channel, "name": "openloop-test"}}
    ...
    a = SlackAdapter("b", "x")
    a._app = _App()
    assert await a._channel_name("C1") == "openloop-test"
    assert await a._channel_name("C1") == "openloop-test"  # served from cache
    assert calls == ["C1"]  # only one API round-trip
    # public §2.1 wrapper delegates to the cached resolver (no extra call)
    assert await a.resolve_channel_name("C1") == "openloop-test"
    assert calls == ["C1"]
...
def test_new_connectors_connect_and_gate_tools(tmp_path):
    from openloop.connectors import connect_connector, connector_list
...
def _connected_tools(tmp_path, monkeypatch, calls):
    ...
    import openloop.connectors.integration_tools as it
...
def test_registry_has_no_duplicate_names():
    ...
    from openloop.connectors.descriptors import DESCRIPTORS
    from openloop.connectors.tool_defs import TOOL_DEFS
...
def test_notion_read_page_flattens_blocks(tmp_path, monkeypatch):
    import openloop.connectors.integration_tools as it
    from openloop.connectors import accounts
...
def test_outlook_manual_multi_account_keys_by_email(tmp_path, monkeypatch):
    """Locally stored Graph tokens remain account-selectable."""
    import openloop.connectors.integration_tools as it
    from openloop.connectors import accounts

    secrets = SecretStore(tmp_path / "secrets.json")
    for email, tok in (("user@openloop.local", "g1"), ("ops@acme.com", "g2")):
        accounts.add_account(
            secrets, "outlook", email, {"access_token": tok, "account": email}
        )
    ids = [a for a, _ in accounts.list_accounts(secrets, "outlook")]
    assert ids == ["ops@acme.com", "user@openloop.local"], ids
    ...
    # default account is the first connected mailbox.
    out = tools["outlook_list_events"]()
    assert out["account"] == "user@openloop.local"
...
def test_outlook_calendar_tools_hit_the_right_graph_endpoints(tmp_path, monkeypatch):
    ...
    accounts.add_account(
        ...
        "outlook",
        "user@openloop.local",
        {"access_token": "tok", "account": "user@openloop.local"},
    )
...
def test_validate_whoami_helper(monkeypatch):
    import openloop.connectors.descriptors as d
...
def test_experimental_hidden_until_enabled(tmp_path, experimental_descriptor):
    from openloop.connectors import connector_list, set_experimental_enabled
...
def test_experimental_package_loads_cleanly():
    ...
    from openloop.connectors.descriptors import DESCRIPTORS
    from openloop.connectors.experimental import EXPERIMENTAL_DESCRIPTORS
