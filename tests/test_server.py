...
def test_persona_routes_are_removed(tmp_path):
    client = _client(tmp_path, [])
    assert client.get("/v1/personas").status_code == 404
    assert client.get("/v1/personas/ops").status_code == 404
    assert client.post("/v1/personas/install", json={}).status_code == 404


def test_pending_routes_replace_inbox_routes(tmp_path):
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    manager.session_store.save(
        SessionRecord("s1", str(tmp_path), "m", "interactive", [], agent="openloop")
    )
    manager.pending.add_approval("s1", "Approve it?")
...
def test_audit_clear_rest(tmp_path):
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    manager.audit_store.append({"tool": "browser_open_url", "stage": "finished"})
    client = TestClient(create_app(manager))
    assert client.get("/v1/audit").json()["events"]
    assert client.delete("/v1/audit").json() == {"ok": True}
    assert client.get("/v1/audit").json()["events"] == []
...
def test_project_remove_hides_without_unbinding_sessions(tmp_path):
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    project_dir = tmp_path / "content"
    project_dir.mkdir()
    project = manager.create_project("内容策略平台", str(project_dir))["project"]
    ...
    removed = client.patch(f"/v1/projects/{pid}", json={"hidden": True}).json()
    assert removed["ok"] is True

    assert client.get("/v1/projects").json()["projects"] == []
    hidden = client.get(
        "/v1/projects", params={"include_hidden": "1"}
    ).json()["projects"]
    assert hidden[0]["hidden"] is True
    assert hidden[0]["unarchived_sessions"] == 1
    assert manager.session_store.load("s-project").project_id == pid


def test_project_create_reopens_hidden_project_at_same_path(tmp_path):
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    project_dir = tmp_path / "content"
    project_dir.mkdir()
    project = manager.create_project("内容策略平台", str(project_dir))["project"]
    pid = project["project_id"]
    manager.update_project(pid, hidden=True)
    client = TestClient(create_app(manager))

    reopened = client.post(
        "/v1/projects",
        json={"name": "内容策略平台", "path": str(project_dir)},
    ).json()

    assert reopened["ok"] is True
    assert reopened["reopened"] is True
    assert reopened["project"]["project_id"] == pid
    assert reopened["project"]["hidden"] is False


def test_sessions_exclude_hidden_project_conversations(tmp_path):
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    project_dir = tmp_path / "content"
    project_dir.mkdir()
    project = manager.create_project("内容策略平台", str(project_dir))["project"]
    ...
    assert manager.session_store.load("hidden-session").project_id == pid


def test_project_relocate_updates_path_without_renaming_project(tmp_path):
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
...
def test_archived_sessions_include_project_recovery_context(tmp_path):
    ...
    manager.session_store.set_flags("archived", archived=True)
    manager.update_project(pid, hidden=True)

    client = TestClient(create_app(manager))
    body = client.get("/v1/sessions/archived").json()
    row = body["sessions"][0]
    assert row["session_id"] == "archived"
    assert row["project_id"] == pid
    assert row["project_name"] == "内容策略平台"
    assert row["project_hidden"] is True
...
def test_ws_rejects_oversized_message(tmp_path):
    from openloop.server import app as app_mod
    from openloop.attachments import MAX_ATTACHMENTS
...
def test_server_sets_explicit_websocket_frame_limit(tmp_path, monkeypatch):
    ...
    from openloop.server import run as server_run
...
def test_standalone_server_token_file_is_user_only(tmp_path, monkeypatch):
    ...
    try:
        assert path == tmp_path / "openloop-state" / "sidecar-9876.token"
        assert path.read_text().strip() == os.environ["OPENLOOP_API_TOKEN"]
        assert len(path.read_text().strip()) == 64
        assert (path.stat().st_mode & 0o777) == 0o600
    finally:
        path.unlink(missing_ok=True)
        os.environ.pop("OPENLOOP_API_TOKEN", None)
...
def test_sidecar_token_gates_rest_and_websockets(tmp_path, monkeypatch):
    from openloop.mcp.config import global_mcp_path
    from starlette.websockets import WebSocketDisconnect as WSD

    monkeypatch.setenv("OPENLOOP_API_TOKEN", "a" * 64)
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    client = TestClient(create_app(manager))
    ...
    assert client.get(
        "/v1/sessions", headers={"X-OpenLoop-Token": "wrong"}
    ).status_code == 401

    current_headers = {"X-OpenLoop-Token": "a" * 64}
    assert client.get("/v1/health", headers=current_headers).json()[
        "default_workspace"
    ] == str(tmp_path.resolve())
    assert client.get("/v1/sessions", headers=current_headers).status_code == 200
    ...
    with client.websocket_connect(
        "/ws/session/authed-current", subprotocols=["openloop", "a" * 64]
    ) as ws:
        assert ws.accepted_subprotocol == "openloop"
        assert ws.receive_json()["type"] == "ready"

    with client.websocket_connect(
        "/ws/events", subprotocols=["openloop", "a" * 64]
    ) as ws:
        assert ws.accepted_subprotocol == "openloop"

    # Removed Cloud callbacks do not exist; local MCP OAuth remains tokenless.
    assert client.get(
        "/auth/callback",
...
def test_ws_browser_tool_audit_round_trip(tmp_path):
    ...
    with client.websocket_connect("/ws/session/browser-audit?agent=openloop") as ws:
        assert ws.receive_json()["type"] == "ready"
...
def test_workspace_command_trust_controls_live_engine(tmp_path):
    ...
    (proj / ".openloop").mkdir(parents=True)
    (proj / ".openloop" / "config.toml").write_text(
...
def test_recent_workspaces_exclude_scratch_dirs(tmp_path):
    ...
    mgr.set_session_root(str(tmp_path / "scratch"))
...
def test_delete_session_removes_its_scratch_dir_only(tmp_path):
    ...
    mgr.session_store.save(
        SessionRecord(
            ...
            managed_root=str(scratch.parent),
        )
    )
...
def test_delete_removes_legacy_named_managed_dir(tmp_path):
    from openloop.server.manager import SessionManager
    from openloop.sessions import SessionRecord
    ...
    old_root.mkdir()
    ws = old_root / "2026-0805-sid123"
    ws.mkdir()
    mgr.set_session_root(str(tmp_path / "new-root"))
    mgr.session_store.save(
        SessionRecord(
            "sid123",
            str(ws),
            ...
            [],
            workspace_kind="managed",
            managed_root=str(old_root),
        )
    )

    assert mgr.delete_session("sid123")["ok"]
    assert not ws.exists()


def test_delete_never_removes_project_dir_even_inside_session_root(tmp_path):
    from openloop.server.manager import SessionManager
    from openloop.sessions import SessionRecord
    ...
    assert mgr.delete_session("proj-sid")["ok"]
    assert proj.exists()
...
def test_ws_auto_provisions_session_root_when_no_default_workspace(tmp_path):
    # Manager with no default workspace: openloop sessions get a managed folder.
    manager = SessionManager(
        workspace=None, data_dir=tmp_path, provider=ScriptedProvider([])
    )
    manager.set_session_root(str(tmp_path / "sessions"))
    client = TestClient(create_app(manager))
    with client.websocket_connect("/ws/session/nofolder") as ws:
        first = ws.receive_json()
        assert first["type"] == "ready"
        assert first["data"]["agent"] == "openloop"
        assert first["data"]["workspace"].startswith(str(tmp_path / "sessions"))
...
def test_ws_project_id_uses_project_folder_and_persists_binding(tmp_path):
    from urllib.parse import quote
    ...
    manager = SessionManager(
        ...
        provider=ScriptedProvider([_text("hi from project")]),
    )
    project = manager.create_project("测试项目", str(project_dir))["project"]
    client = TestClient(create_app(manager))

    with client.websocket_connect(
        f"/ws/session/project-bound?workspace={quote(str(other_dir))}&project_id={project['project_id']}"
    ) as ws:
        ready = ws.receive_json()
        assert ready["type"] == "ready"
        assert ready["data"]["workspace"] == str(project_dir.resolve())
        ws.send_json({"type": "user_message", "text": "hello"})
        assert "turn_end" in _drain(ws)

    record = manager.session_store.load("project-bound")
    assert record.workspace == str(project_dir.resolve())
    assert record.project_id == project["project_id"]


def test_ws_ignores_legacy_agent_query_and_uses_openloop(tmp_path):
    manager = SessionManager(
        workspace=None,
        data_dir=tmp_path,
        provider=ScriptedProvider([_text("hi from openloop")]),
    )
    manager.set_session_root(str(tmp_path / "sessions"))
    client = TestClient(create_app(manager))
    with client.websocket_connect("/ws/session/chat1?agent=chat") as ws:
        ready = ws.receive_json()
        assert ready["type"] == "ready"
        assert ready["data"]["agent"] == "openloop"
        assert ready["data"]["workspace"].startswith(str(tmp_path / "sessions"))
        ws.send_json({"type": "user_message", "text": "hello"})
        assert "turn_end" in _drain(ws)
...
def test_session_messages_prefers_the_live_engine(tmp_path):
    ...
    mgr = client.app.state.manager
    engine = mgr.get_engine("__run__live", agent="openloop")
...
def test_always_allow_grants_survive_restart(tmp_path):
    ...
    def _run_turn(client, expect_prompts):
        with client.websocket_connect("/ws/session/grants1?agent=openloop") as ws:
            assert ws.receive_json()["type"] == "ready"
...
def test_google_connectors_are_manual_only(tmp_path):
    client = _client(tmp_path, [])
    connectors = {c["name"]: c for c in client.get("/v1/connectors").json()["connectors"]}
    for name in ("gmail", "google_calendar", "google_drive"):
        c = connectors[name]
        assert "managed" not in c and "managed_paused" not in c
        assert c["fields"], f"{name} lost its manual fields"

    assert client.post("/v1/connectors/gmail/connect-managed", json={}).status_code == 404
