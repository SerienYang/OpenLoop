...
from openloop.automation import (
    Schedule,
    ScheduledTask,
    Scheduler,
    TaskRun,
...
async def test_scheduled_run_persists_continuable_session(tmp_path, monkeypatch):
    ...
    from openloop.server.manager import SessionManager, _last_assistant_text
...
async def test_manual_run_prepare_and_finalize(tmp_path, monkeypatch):
    ...
    engine = manager.get_engine(prep["session_id"], workspace=str(ws), agent="openloop")
...
async def test_scheduled_run_broadcasts_run_started_event(tmp_path, monkeypatch):
    ...
    automation_run_started (the top-right toast). The run also drives app-wide
    running-state events for desktop keep-awake. Dead sockets drop silently."""
    from openloop.providers import AssistantTurn, ModelCapabilities, ProviderClient
    from openloop.server.manager import SessionManager
    ...
    states = [m for m in heard if m["type"] == "running_state_changed"]
    assert states[0]["data"] == {"running": True, "count": 1}
    assert states[-1]["data"] == {"running": False, "count": 0}
