...
model actually reads the image (`OPENLOOP_LIVE_VISION=1`, key read from the SecretStore).
"""
...
def test_list_content_message_persists_and_titles(tmp_path):
    from openloop.conversations import ConversationStore, title_from
...
@pytest.mark.skipif(
    os.environ.get("OPENLOOP_LIVE_VISION") != "1",
    reason="opt-in: real OpenAI vision call (set OPENLOOP_LIVE_VISION=1)",
)
...
def test_pdf_attachment_invalid_or_oversized_skipped():
    from openloop.attachments import MAX_PDF_CHARS
