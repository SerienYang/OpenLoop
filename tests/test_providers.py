...
def test_compat_vendor_descriptors_ship_prefilled_endpoints():
    from openloop.providers.registry import get_descriptor
...
def test_compat_builder_defaults_and_profile_override(monkeypatch):
    from openloop.providers.registry import build_provider_client
...
def test_compat_models_route_and_get_tool_capabilities():
    from openloop.providers.router import ProviderRouter
...
def test_opencode_go_descriptor_builder_and_matrix_are_complete(monkeypatch):
    from openloop.providers.matrix import MATRIX, models_for_provider
    ...
    from openloop.providers.registry import build_provider_client, get_descriptor
    from openloop.server.manager import SessionManager

    descriptor = get_descriptor("opencode-go")
    assert descriptor is not None
    assert descriptor.title == "OpenCode Go"
    assert descriptor.env_key == "OPENCODE_GO_API_KEY"
    assert descriptor.recommended_model == "deepseek-v4-flash"
    assert descriptor.needs_key
    assert [(field.key, field.secret, field.required) for field in descriptor.fields] == [
        ("api_key", True, True)
    ]
    assert "subscription" in descriptor.blurb.lower()

    official = CHAT_MODELS | RESPONSES_MODELS | MESSAGES_MODELS
    assert set(models_for_provider("opencode-go")) == official
    assert {
        key.removeprefix("opencode-go:") for key in MATRIX if key.startswith("opencode-go:")
    } == official
    assert descriptor.recommended_model in SessionManager.COMPAT_MODELS["opencode-go"]

    built = build_provider_client("opencode-go", {"api_key": "go-key"}, None)
    assert isinstance(built, OpenCodeGoProvider)
    assert built._api_key == "go-key"

    monkeypatch.setenv("OPENCODE_GO_API_KEY", "env-go-key")
    from_env = build_provider_client("opencode-go", {}, None)
    assert from_env._api_key == "env-go-key"
...
def test_matrix_labels_and_custom_model_fallback():
    from openloop.providers.matrix import MATRIX, model_labels
...
def test_reseller_descriptors_and_matrix_stay_in_lockstep():
    ...
    from openloop.providers.matrix import models_for_provider
    from openloop.providers.registry import get_descriptor
