...
from openloop.providers.registry import _normalize_ollama_url, build_provider_client
from openloop.providers.openai_provider import _salvage_tool_calls_from_text
...
def _patch_build(monkeypatch):
    ...
    monkeypatch.setattr("openloop.providers.router.build_provider_client", fake_build)
    return state
...
def test_manager_curated_models(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("OPENLOOP_STATE_DIR", str(tmp_path / "state"))
    from openloop.providers.registry import provider_descriptors
...
def test_provider_builders(monkeypatch):
    ...
    from openloop.providers import AnthropicProvider, GeminiProvider
    ...
    from openloop.providers import OpenAIResponsesProvider
...
def test_settings_omit_legacy_surface_preferences(tmp_path, monkeypatch):
    monkeypatch.setenv("OPENLOOP_STATE_DIR", str(tmp_path / "state"))
    from openloop.server.manager import SessionManager

    mgr = SessionManager(data_dir=tmp_path)
    assert "surfaces" not in mgr.get_settings()
    assert not hasattr(mgr, "set_surfaces")
