...
from openloop.connectors import ConnectorSettings, Gateway
from openloop.connectors.base import InteractionEvent, MessageEvent, SessionSource
from openloop.interactions import encode
...
def test_manual_owner_rest_flow_surfaces_identity_and_binding(tmp_path):
    ...
    bound = client.post(
        "/v1/connectors/slack/pending-routing/binding",
...
def test_slack_reply_token_cannot_resolve_approvals_for_any_actor(tmp_path):
    manager = _manager(tmp_path)
    _manual_profile(
        manager,
        allowed_users=["U_OWNER", "U_MEMBER"],
    ...
    assert manager.inbox.get(item.id).state == "pending"
    assert manager.inbox.get(item.id).resolution is None


def test_ownerless_binding_does_not_mirror(tmp_path):
    manager = _manager(tmp_path)
