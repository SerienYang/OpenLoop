...
from openloop.web import guard
from openloop.web.fetch import make_web_fetch_tool
...
def test_guarded_backend_connects_to_the_checked_ip_not_the_hostname(monkeypatch):
    """The actual socket target must be the public answer that passed validation.

    Passing the hostname through would make httpcore resolve it a second time, reopening
    DNS rebinding between check_url() and connect_tcp().
    """
    _resolves_to(monkeypatch, "93.184.216.34")
    raw = _RecordingBackend()
    backend = guard.GuardedNetworkBackend(raw)

    backend.connect_tcp("rebind.example", 443)

    assert raw.hosts == [("93.184.216.34", 443)]


def test_guarded_backend_rejects_a_private_answer_before_connect(monkeypatch):
    _resolves_to(monkeypatch, "127.0.0.1")
    raw = _RecordingBackend()
    backend = guard.GuardedNetworkBackend(raw)
