...
from openloop.connectors.adapters import SlackAdapter
from openloop.connectors.base import InteractionEvent, MessageEvent
from openloop.connectors.config import ConnectorSettings
from openloop.connectors.gateway import Gateway
from openloop.interactions import Button
...
async def test_send_and_send_interactive_recorded(fake_slack):
    ...
    assert [e["action_id"] for e in elements] == ["openloop_0", "openloop_1"]


# 4. An `openloop_*` button click → adapter action handler → gateway interaction handler
...
async def test_real_bolt_dispatches_both_envelope_shapes(fake_slack):
    ...
    @app.action(re.compile(r"^openloop_"))
    ...
    try:
        ...
        assert seen["action"]["actions"][0]["action_id"] == "openloop_0"
...
#    Slack encodes "@openloop hi"), so parked cards / transcripts never show raw ids. Unresolvable
...
async def test_inbound_rewrites_mention_tokens(fake_slack):
    ...
    fake_slack.add_user("UOPENLOOP99", "openloop", display_name="openloop")
    fake_slack.add_channel("C1", "general", is_im=False)
    ...
    try:
        ...
        await fake_slack.inbound(
            channel="C1", user="U1", text="<@UOPENLOOP99> hi — ask <@UGHOST99> too"
        )
        await asyncio.wait_for(delivered.wait(), timeout=5)
        # The known mention resolves; the unknown id keeps its token.
        assert got[0].text == "@openloop hi — ask <@UGHOST99> too"
    finally:
        await adapter.disconnect()
