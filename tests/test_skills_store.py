...
from openloop.skills import SkillLoader, SkillStore, validate_name
...
def test_create_project_scoped(store, workspace):
    ...
    md = workspace / ".openloop" / "skills" / "release-checklist" / "SKILL.md"
    assert md.is_file()
...
def test_move_roundtrip(store, workspace):
    ...
    assert (workspace / ".openloop" / "skills" / "mover" / "SKILL.md").is_file()
...
def test_move_collision_leaves_source(store, workspace):
    ...
    assert (workspace / ".openloop" / "skills" / "both" / "SKILL.md").is_file()
...
from openloop.skills import save_skill_tool  # noqa: E402
...
def test_save_skill_requires_approval_metadata(store):
    ...
    assert tool.__openloop_schema__["function"]["name"] == "save_skill"
    required = tool.__openloop_schema__["function"]["parameters"]["required"]
