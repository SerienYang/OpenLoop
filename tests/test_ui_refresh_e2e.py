...
1. Connect Slack against FakeSlack through the manager's gateway; allow a user; subscribe an
   OpenLoop incident session to a channel.
...
4. Verify the removed per-session connector route cannot override global connector access.

State is fully isolated: `OPENLOOP_STATE_DIR` redirects the SecretStore and the manager's data
...
from openloop.interactions import decode
...
from openloop.server import create_app
...
def _approve_button(card):
    """The first action button (`openloop_0` = Approve) of a mirrored approval card."""
    for block in card.get("blocks") or []:
        if block.get("type") == "actions":
            for el in block.get("elements") or []:
                if el.get("action_id") == "openloop_0":
                    return el
    return None
...
async def test_ui_refresh_cross_cutting_e2e(fake_slack, tmp_path, monkeypatch):
    ...
    # by what this test writes; the manager's own data dir lives under tmp_path/.openloop.
    ...
    try:
        ...
        send_card = await _wait_until(
            lambda: next(
                (
                    msg
                    for msg in fake_slack.outbound()
                    if "send_message" in (msg.get("text") or "") and msg.get("blocks")
                ),
                None,
            )
        )
        assert send_card is not None, f"send_message approval never mirrored: {fake_slack.outbound()}"
        send_button = _approve_button(send_card)
        assert send_button is not None and send_button["value"]
        await fake_slack.interaction(
            channel=CHANNEL,
            user=USER,
            username="alice",
            message_ts=send_card["ts"],
            action_id=send_button["action_id"],
            value=send_button["value"],
        )
        ...
        # -- Step 4: connector access is global; the retired per-session route stays absent ---------
        resp = client.post(
            f"/v1/sessions/{SID}/connections",
            json={"connector": "slack", "enabled": False},
        )
        assert resp.status_code == 404
        assert "slack" in mgr.effective_connectors(SID)
    finally:
        await mgr.aclose()

    # State-dir isolation held: the SecretStore resolved to the tmp_path-scoped path, never the
    # machine-global ~/.config/openloop (so this run cannot mutate real state).
