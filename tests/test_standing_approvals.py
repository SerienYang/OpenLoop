...
from openloop.automation import Schedule, ScheduledTask, Scheduler, TaskRun, TaskStore
from openloop.automation.models import grant_entries, rule_entry, rule_parts
from openloop.automation.tools import scheduling_tools
from openloop.engine import ApprovalOutcome, PermissionRequest
from openloop.permissions import Mode, PermissionEngine, standing_rule_candidate
...
def test_background_connector_task_rules_require_approval(tmp_path):
    e = PermissionEngine(
        workspace_root=tmp_path,
        task_rules={"send_message": {"slack:T1/C1"}},
        force_connector_approval=True,
    )
    d = e.evaluate(
        "send_message",
        {"target": "slack:T1/C1", "text": "digest"},
        _Meta(category="messaging"),
    )
    assert not d.allowed and d.needs_user
    assert "background" in d.reason
...
def test_create_tool_stores_write_grants(tmp_path):
    ...
    props = create.__openloop_schema__["function"]["parameters"]["properties"]
    ...
    assert (
        "permissions"
        not in update.__openloop_schema__["function"]["parameters"]["properties"]
    )
...
async def test_scheduled_approver_parks_and_mints(tmp_path, monkeypatch):
    ...
    task = _task(workspace=str(ws), agent="openloop")
    ...
    # …and a rebuilt run engine records the rule, but background connector writes still park.
    ...
    assert not hit.allowed and hit.needs_user
    assert "background" in hit.reason
...
async def test_scheduled_approver_name_allows_and_denies(tmp_path, monkeypatch):
    ...
    ws.mkdir()
    manager = SessionManager(data_dir=tmp_path / "data", provider=_provider())
    task = _task(workspace=str(ws), agent="openloop", always_allowed_tools=["web_search"])
    manager.task_store.save(task)
    run = TaskRun(task_id=task.id)
    manager.task_store.add_run(run)
    approver = manager._scheduled_approver(task, run.session_id)
...
async def test_scheduled_connector_name_allow_still_parks(tmp_path, monkeypatch):
    from openloop.server.manager import SessionManager
    ...
    task = _task(
        workspace=str(ws),
        agent="openloop",
        always_allowed_tools=["send_message"],
    )
    manager.task_store.save(task)
    run = TaskRun(task_id=task.id)
    manager.task_store.add_run(run)
    ...
    outcome, _ = await asyncio.gather(approver(request), allow_once())
    assert outcome is ApprovalOutcome.ONCE
...
def test_get_engine_seeds_run_session_rules(tmp_path, monkeypatch):
    ...
    engine = manager.get_engine(run.session_id, workspace=str(ws), agent="openloop")
    assert engine.permissions.task_rules == {"send_message": {"slack:T1/C1"}}
...
def test_engine_events_carry_standing_context(tmp_path):
    ...
    from openloop.engine import TurnEngine
    ...
    send_message.__openloop_schema__ = {
        "type": "function",
        "function": {
            "name": "send_message",
            "description": "send",
