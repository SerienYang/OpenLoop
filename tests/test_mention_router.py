...
@OpenLoop tagged in a channel is the PRIMARY entry point: with no subscribed session, a
per-thread openloop session spawns (visible in the sidebar, thread-scoped standing
send_message grant); follow-up tags in the same thread steer the same session. A
channel with a user-connected (subscribed) openloop overrides the router — it must
...
from openloop.connectors.adapters import slack_event_to_event
from openloop.connectors.base import MessageEvent, SessionSource
...
def test_slack_mapper_computes_mentions_me():
    ...
    assert slack_event_to_event(
        {**base, "text": "hey <@UBOT|OpenLoop> hi"}, "UBOT"
    ).mentions_me
...
def test_subscribed_openloop_overrides_router(tmp_path, monkeypatch):
    mgr = _mgr(tmp_path)
    captured = _capture_deliveries(mgr, monkeypatch)
    ...
    # Delivered to the connected openloop with must-respond framing + the thread target…
...
def test_origin_columns_migrate_on_old_db(tmp_path):
    ...
    db = tmp_path / "openloop.db"
    conn = sqlite3.connect(db)
    conn.execute(
        "CREATE TABLE sessions (session_id TEXT PRIMARY KEY, workspace TEXT, model TEXT, "
        "mode TEXT, title TEXT, agent TEXT DEFAULT 'openloop', n_msgs INTEGER DEFAULT 0, "
