...
class CompactingProvider(ProviderClient):
    ...
    def complete(self, *, model, messages, tools=None, **settings):
        if messages and "compacting an OpenLoop session" in str(
...
def test_compaction_state_survives_save_and_rebuild(tmp_path):
    from openloop.compaction import CompactionState
    from openloop.server.manager import SessionManager
    ...
    rebuilt = mgr.get_engine(sid, agent="openloop", workspace=str(tmp_path))
