...
Accounts live at `gmail:account:<email>`; `gmail:default` is just the default
pointer + filters. The
...
from openloop.connectors import gmail_accounts
...
def test_second_account_added_first_stays_default(secrets):
    gmail_accounts.connect_account(secrets, _account("one@x.com"))
    gmail_accounts.connect_account(secrets, _account("two@y.com"))
...
def test_search_omits_filtered_senders_and_counts_hidden(secrets, monkeypatch):
    gmail_accounts.connect_account(secrets, _account("me@x.com"))
