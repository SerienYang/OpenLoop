...
Portals live at `hubspot:portal:<hub_id>`; `hubspot:default` is the default pointer + the
...
from openloop.connectors import hubspot_portals
from openloop.connectors.integration_tools import make_integration_tools
from openloop.connectors.setup import connector_list
...
def test_portals_list_with_access_and_sandbox(secrets):
    hubspot_portals.connect_portal(secrets, _portal("111", sandbox=True))
    hubspot_portals.connect_portal(
        secrets,
...
def test_default_repoints_on_disconnect(secrets):
    hubspot_portals.connect_portal(secrets, _portal("111"))
    hubspot_portals.connect_portal(secrets, _portal("222"))
...
def _fake_hubspot(monkeypatch, responses: dict[str, dict]):
    from openloop.connectors import integration_tools
...
def test_portal_routes_default_and_disconnect(client):
    for hub in ("111", "222"):
        hubspot_portals.connect_portal(client.manager.secrets, _portal(hub))
