...
from openloop.connectors.base import MessageEvent, MessageSource, SessionSource
from openloop.engine import TurnEngine
from openloop.permissions import PermissionEngine
...
def test_message_source_persisted_and_stripped(tmp_path):
    ...
    _connect_slack(mgr)
    mgr.get_engine("S", agent="openloop")  # durable, workspace-free session
    ...
    assert last_user["source"]["channel_name"] == "#openloop-test"
...
def test_tool_display_sidecar_is_agent_invisible(tmp_path):
    ...
    from openloop.providers.base import ToolCall
...
def test_turn_start_carries_source(tmp_path):
    ...
    _connect_slack(mgr)
    mgr.get_engine("S", agent="openloop")
    mgr.subscriptions.subscribe("S", "slack:C1")
    ...
    assert source["channel_name"] == "#openloop-test"
