...
Accounts live at `google_calendar:account:<email>`; `google_calendar:default` is the default
...
from openloop.connectors import gcal_accounts
from openloop.connectors.integration_tools import make_integration_tools
from openloop.connectors.setup import connector_list, disconnect_connector
...
def test_second_account_added_first_stays_default(secrets):
    gcal_accounts.connect_account(secrets, _account("one@x.com"))
    gcal_accounts.connect_account(secrets, _account("two@y.com"))
...
def test_update_event_patches_only_provided_fields(secrets, monkeypatch):
    gcal_accounts.connect_account(secrets, _account("me@x.com"))
    calls = _fake_gcal(
