...
(conversations.list), cached for the Socket Mode workspace and ranked locally.
...
def test_members_filtered_ranked_and_guest_tagged(secrets, monkeypatch):
    ...
    out = slack_directory.list_members(secrets, "default")
    assert out["ok"]
    ...
    out = slack_directory.list_members(secrets, "default", query="ro")
    assert [m["id"] for m in out["members"]] == ["U2"]  # prefix beats substring
    out = slack_directory.list_members(
        secrets, "default", query="maya"
    )  # handle matches too
    assert [m["id"] for m in out["members"]] == ["U1"]


def test_roster_is_cached_for_the_workspace(secrets, monkeypatch):
    calls = _fake_pages(monkeypatch, {"users.list": MEMBERS})
    slack_directory.list_members(secrets, "default")
    slack_directory.list_members(secrets, "default", query="ro")
    assert len(calls) == 1
    assert calls[0][1] == "xoxb-manual"
...
def test_channels_carry_privacy_and_membership(secrets, monkeypatch):
    ...
    out = slack_directory.list_channels(secrets, "default", query="l")
    assert out["ok"]
...
def test_allow_with_name_seeds_people_directory(tmp_path, monkeypatch):
    ...
    manager.secrets.put("slack:default", {"bot_token": "xoxb", "allowed_users": []})
