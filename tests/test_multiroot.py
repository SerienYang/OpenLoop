...
Regular OpenLoop sessions own a primary writable scratch dir and may gain additional folders,
...
from openloop.permissions import Decision, Mode, PermissionEngine
from openloop.providers import AssistantTurn, ToolCall
from openloop.roots import RootDir, normalize_roots, render_context
...
def _openloop_manager(tmp_path):
    from openloop.providers import ModelCapabilities, ProviderClient
    from openloop.server import SessionManager
    ...
    mgr.set_session_root(str(tmp_path / "scratchbase"))
    return mgr


def test_add_and_remove_roots_live_and_persisted(tmp_path):
    mgr = _openloop_manager(tmp_path)
    ...
    mgr.save(sid, engine)
    mgr2 = _openloop_manager(tmp_path)
...
def test_add_root_before_first_turn_persists(tmp_path):
    ...
    the manager creates a minimal openloop record so the grant isn't lost (GUI start panel).
    """
    mgr = _openloop_manager(tmp_path)
    ...
    # the grant survived: a fresh manager (no engines) still sees it, under openloop
    mgr2 = _openloop_manager(tmp_path)
    persisted = {r["path"]: r for r in mgr2.get_roots(sid)}
    assert str(shared.resolve()) in persisted
    record = mgr2.session_store.load(sid)
    assert record is not None and record.agent == "openloop"
