...
def test_no_workspace_means_global_only(manager, tmp_path):
    ...
    (ws / ".openloop" / "skills").mkdir(parents=True)
    _skill(ws / ".openloop" / "skills", "local-only")
...
def test_empty_catalog_is_safe(tmp_path):
    from openloop.tools.registry import ToolRegistry
...
def test_live_load_skill_semantics(manager):
    ...
    from openloop.agents.registry import get_agent

    _skill(manager.skill_store.global_dir, "early", body="early body")
    engine = build_engine(
        agent=get_agent("openloop"),
