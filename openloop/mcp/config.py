...
Global:    ~/.config/openloop/mcp.json
Workspace: <workspace>/.openloop/mcp.json   (overrides global on name clash,
...
def _config_paths(
    ...
    so an untrusted repo's `.openloop/mcp.json` is never read — cloning alone must
    not be enough to define processes that run at session open.
    """
    paths = [global_mcp_path()]
    if workspace and workspace_trusted:
        paths.append(Path(workspace).expanduser() / ".openloop" / "mcp.json")
    return paths
