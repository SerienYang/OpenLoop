...
from .guard import get_checked, guarded_http_transport
...
def make_web_fetch_tool() -> Callable[..., Any]:
    def web_fetch(url: str, max_chars: int = _MAX) -> dict[str, Any]:
        ...
        try:
            ...
            # address-checked. The custom transport pins each TCP connection to that
            # checked IP, closing the DNS-rebinding gap between validation and connect.
            with httpx.Client(
                transport=guarded_http_transport(),
                follow_redirects=False,
                timeout=20.0,
                headers={"User-Agent": "openloop/0.1 (+desktop)"},
            ) as client:
    ...
    web_fetch.__openloop_schema__ = _SCHEMA
    return web_fetch
