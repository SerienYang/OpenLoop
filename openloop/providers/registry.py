...
def _build_opencode_go(profile: dict[str, Any], secrets: Any) -> ProviderClient:
    ...
    if not api_key:
        raise RuntimeError(
            "No OpenCode Go API key configured — add it in Settings ▸ Models."
        )
    return OpenCodeGoProvider(api_key=api_key)
    ...
    ProviderDescriptor(
        ...
        fields=[
            ProviderField(
                ...
                "OpenCode Go API key",
                secret=True,
                help="Get a workspace API key at https://opencode.ai/auth",
            ),
        ],
        build=_build_opencode_go,
        recommended_model="deepseek-v4-flash",
        env_key="OPENCODE_GO_API_KEY",
        blurb="A subscription gateway for a curated set of coding models.",
    ),
    ProviderDescriptor(
        name="bedrock",
        title="AWS Bedrock",
...
def _verify_bedrock(fields: dict[str, Any], timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "error": "boto3 is not installed — `pip install 'openloop[bedrock]'`.",
        }
