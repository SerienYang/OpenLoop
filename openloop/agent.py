"""Engine assembly for an OpenLoop Agent.
...
# for the runtime); models that ignore it degrade gracefully to a turn with no narration.
_NARRATION_GUIDANCE = """\
...
def _skill_dirs(workspace: Optional[Path]) -> list[Path]:
    ...
    if workspace is not None:
        dirs.append(workspace / ".openloop" / "skills")
    return dirs
...
def build_engine(
    *,
    ...
    force_connector_approval: bool = False,
    ...
    # The session's directories. Explicit roots (scratch + added folders) win;
    ...
    # Messaging connectors expose send_message when at least one channel is configured.
    ...
    # Workspace sessions can ask the user mid-task for another folder.
    if root_list:
    ...
    # Workspace sessions can fan broad research out to read-only explorer subagents, keeping
    ...
    # Scheduling: workspace sessions can set up scheduled tasks (origin = this session).
    if task_store is not None and ws is not None:
        origin = {
            "surface": agent.name,
    ...
    # Self-wake: sessions can suspend + schedule their own resumption (timer /
    # on-completion / on-event). The scheduler tick resumes due wakes.
    if wake_store is not None and session_id:
    ...
    # User-local risk overrides (mainly to relax MCP's conservative default).
    ...
    # directory list (OpenLoop can gain folders mid-session).
    roots_context = (
