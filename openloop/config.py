...
Workspace: <workspace>/.openloop/config.toml   (overrides global)
...
def workspace_allowed_commands(workspace: str | Path) -> list[str]:
    ...
    path = Path(workspace).expanduser() / ".openloop" / "config.toml"
...
def load_config(
    ...
    *,
    ...
    if workspace:
        w = Path(workspace).expanduser() / ".openloop" / "config.toml"
        if w.is_file():
