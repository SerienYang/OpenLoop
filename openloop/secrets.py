...
`${ENV_VAR}` references resolved at read time from the process env / `~/.config/openloop/.env`.
...
def state_dir() -> Path:
    """Where OpenLoop keeps its state — the one cross-platform source of truth.

    Resolution order:
    1. `$OPENLOOP_STATE_DIR` — current explicit override on any OS.
    2. Windows: `%APPDATA%\\openloop` (e.g. `C:\\Users\\You\\AppData\\Roaming\\openloop`),
       the native per-user app-data location.
    3. macOS / Linux: `~/.config/openloop`.
    """
    base = os.environ.get("OPENLOOP_STATE_DIR")
    if base:
        return Path(base).expanduser()
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        if appdata:
            return Path(appdata) / "openloop"
    return Path.home() / ".config" / "openloop"
...
class SecretStore:
    ...
    def get_raw(self, profile: str) -> Optional[dict[str, Any]]:
        """Return an unresolved profile copy for transactional compensation."""
        with self._lock:
            data = self._read().get(profile)
    ...
    def restore_raw(self, profile: str, data: Optional[dict[str, Any]]) -> None:
        """Restore a raw snapshot, or remove the profile when the snapshot was absent."""
        with self._lock:
            store = self._read()
            if data is None:
                store.pop(profile, None)
            else:
                store[profile] = json.loads(json.dumps(data))
            self._write(store)
