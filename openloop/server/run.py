"""Launch the server with uvicorn. Used by the desktop GUI sidecar and `openloop-server`."""
...
def _exit_when_orphaned() -> None:
    """When launched as a desktop sidecar (`OPENLOOP_EXIT_WITH_PARENT=1`), exit if the parent
    process dies — even on an abrupt kill (e.g. the Tauri dev watcher restarting the app, or a
    crash) that skips the shell's graceful child-kill. Standalone server runs are
    unaffected.

    The GUI passes its own PID in `OPENLOOP_PARENT_PID`. Watching that explicit PID (not
    ...
    *grandchild* of the GUI — the bootloader sits in between, so getppid() points at the
    ...
    """
    if os.environ.get("OPENLOOP_EXIT_WITH_PARENT") != "1":
        return
    import threading

    try:
        parent = int(os.environ.get("OPENLOOP_PARENT_PID") or 0)
    except ValueError:
        parent = 0
...
def _ensure_api_token(port: int) -> Path | None:
    ...
    supplied = os.environ.get("OPENLOOP_API_TOKEN")
    if supplied:
        os.environ["OPENLOOP_API_TOKEN"] = supplied
        return None  # Tauri supplied an in-memory token; never persist it.
    token = secrets.token_hex(32)
    os.environ["OPENLOOP_API_TOKEN"] = token
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="openloop-server")
    ...
    # Publish the actual bound port for local MCP OAuth callbacks. The desktop
    # shell uses a random free port so it can coexist with a hand-run server.
    os.environ["OPENLOOP_PORT"] = str(args.port)
