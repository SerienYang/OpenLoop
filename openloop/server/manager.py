...
class SessionManager:
    def __init__(
        ...
        *,
        ...
        if data_dir is not None:
            ...
            base = Path(self.default_workspace) / ".openloop"
        else:
            base = state_dir()
        base.mkdir(parents=True, exist_ok=True)

        self.memory_store: MemoryStore = SQLiteMemoryStore(base / "openloop.db")
        self.audit_store = AuditStore(base / "openloop.db")
        self.audit_store.purge_expired()
        self.session_store = ConversationStore(base)
        self.session_store.canonicalize_workspaces()  # collapse /tmp vs /private/tmp etc.
        self.session_store.migrate_workspace_kinds(
            {p["path"] for p in self.session_store.list_projects(include_hidden=True)}
        )
        ...
        self._recover_provider_config_journal()
        self._ensure_provider_order()
        ...
        # Pending (cross-session human-attention queue), routing (named inboxes + Slack/Telegram
        # bindings), the Unattended toggle, and self-wake records.
        self.pending = PendingStore(base / "pending.json", legacy_path=base / "inbox.json")
        self.inbox = self.pending  # transitional alias until routes are renamed in Phase 3.2
        ...
        # Connector credentials are global in the unified runtime. Remove the retired
        # per-session override file so an old mute cannot silently survive the migration.
        (base / "session_connections.json").unlink(missing_ok=True)
    ...
    def _mcp_workspace_trusted(self, workspace: Optional[str | Path]) -> bool:
        """Whether workspace `.openloop/mcp.json` may be loaded (#213).
    ...
    def recent_workspaces(self) -> list[dict[str, Any]]:
        ...
        scratch = self.session_root().resolve()
        out = []
    ...
    def project_workspace(self, project_id: Optional[str]) -> Optional[str]:
        ...
        return str(project.get("path") or "") if project else None

    def list_archived_sessions(self) -> list[dict[str, Any]]:
        return self.session_store.list_archived_sessions()

    def default_dir(self) -> str:
        """Where new folders should default to (folder picker / gate prefill)."""
        return (
            self._prefs.get("default_dir")
            or self._prefs.get("session_root")
            or self.DEFAULT_SESSION_ROOT
        )

    DEFAULT_SESSION_ROOT = "~/OpenLoop"

    def session_root(self) -> Path:
        """Common area for per-conversation scratch directories. Configurable via prefs."""
        base = self._prefs.get("session_root") or self.DEFAULT_SESSION_ROOT
        return Path(base).expanduser()

    def scratch_base(self) -> Path:
        """Legacy alias while REST/UI callers are renamed to session_root."""
    ...
    def _provision_scratch(self, session_id: str) -> str:
        ...
        Named ``<YYYY-MM-DD>_<session_id>`` so managed session dirs self-sort by
        creation date under the session root.
        """
        d = self.session_root() / f"{time.strftime('%Y-%m-%d')}_{session_id}"
        ...
        *,
        ...
        if record:
            ...
            if project_id and not project:
                return None
            ws = (
                self.resolve_workspace(project["path"])
                if project
                else self.resolve_workspace(workspace)
                if ag.needs_workspace
                else None
            )
        ...
        # The scratch workspace is the primary writable root, plus any persisted folders.
        roots = None
        ...
        mention_targets = self.mention_sessions.targets_for(session_id)
        force_connector_approval = (
            self.unattended.is_unattended(session_id)
            or owning_task is not None
            or bool(mention_targets)
        )
        engine = build_engine(
            agent=ag,
            workspace=ws,
            ...
            connector_filter=self.effective_connectors(session_id),
            force_connector_approval=force_connector_approval,
        ...
        for thread_target in mention_targets:
    ...
    def _routing_targets(self, session_id: str, agent: str) -> list[str]:
        ...
        binding = self.inbox_routing.binding_for(
            self.inbox_routing.route_for(session_id)
        )
        return [f"{binding.channel}:{binding.target}"] if binding.channel else []

    def effective_connectors(self, session_id: str) -> set[str]:
        """All connected connectors are available to every OpenLoop session."""
    ...
    def inbox_question_asker(self, session_id: str, agent: str):
        ...
        async def ask(
            ...
            inbox_name = self.inbox_routing.route_for(session_id)
    ...
    def inbox_approver(self, session_id: str, agent: str):
        ...
        async def approve(request):
            item = self.inbox.add_approval(
                ...
                inbox=self.inbox_routing.route_for(session_id),
        ...
        for server in load_mcp_servers(
            ...
            if backed:
                ...
                if effective is None:
                    effective = self.effective_connectors(session_id)
    ...
    def clear_audit(self) -> dict[str, Any]:
    ...
    def list_artifacts(self, session_id: str) -> list[dict[str, Any]]:
        ...
        # TCC prompt ("OpenLoop would like to access data from other apps") on every turn.
    ...
    def _next_provider_order_revision(self, revision: int) -> int:
        if revision >= MAX_SAFE_INTEGER:
            raise ValueError("provider order revision exhausted")
        return revision + 1

    def _ensure_provider_order_locked(
        self,
        descriptors: list[Any],
        configured: Optional[set[str]] = None,
    ) -> tuple[list[str], int]:
        built_in = [descriptor.name for descriptor in descriptors]
        stored = self._prefs.get("provider_order")
        stored_revision = self._prefs.get("provider_order_revision")
        valid_revision = True
        try:
            revision = validate_revision(stored_revision)
        except ValueError:
        ...
        if not isinstance(stored, list):
            ...
            changed = order != stored
            if valid_revision and changed:
                revision = self._next_provider_order_revision(revision)

        applied = normalize_applied_requests(
            self._prefs.get("provider_order_applied_requests")
        )
        applied_changed = applied != self._prefs.get(
            "provider_order_applied_requests", []
        )
        if changed or not valid_revision or applied_changed:
            self._prefs["provider_order"] = order
            self._prefs["provider_order_revision"] = revision
            if applied:
                self._prefs["provider_order_applied_requests"] = applied
            else:
                self._prefs.pop("provider_order_applied_requests", None)
            self._save_prefs()
        return order, revision

    def _ensure_provider_order_under_config_lock(self) -> tuple[list[str], int]:
        descriptors = provider_descriptors()
        with self._prefs_lock:
            needs_configured = not isinstance(
                self._prefs.get("provider_order"),
                list,
            )
        configured: Optional[set[str]] = None
        if needs_configured:
            configured = {
                ...
                if descriptor_configured(
                    descriptor,
                    self.secrets.get(f"provider:{descriptor.name}") or {},
                )
            }
        with self._prefs_lock:
            return self._ensure_provider_order_locked(descriptors, configured)

    def _ensure_provider_order(self) -> tuple[list[str], int]:
        with self._provider_config_lock:
            return self._ensure_provider_order_under_config_lock()
    ...
    def get_provider_order(
        ...
        with self._provider_config_lock:
            order, revision = self._ensure_provider_order_under_config_lock()
            with self._prefs_lock:
                request_applied = applied_request_state(
                    self._prefs.get("provider_order_applied_requests"),
                    request_id,
    ...
    def set_provider_order(
        ...
        requested = validate_provider_ids(providers)
        requested_revision = validate_revision(revision)
        request_id = validate_request_id(request_id)
        with self._provider_config_lock:
            current, current_revision = (
                self._ensure_provider_order_under_config_lock()
            )
            with self._prefs_lock:
                applied = normalize_applied_requests(
                    self._prefs.get("provider_order_applied_requests")
                )
                if any(item["request_id"] == request_id for item in applied):
                    return {
                        "ok": True,
                ...
                built_in = [descriptor.name for descriptor in provider_descriptors()]
                order = normalize_provider_order(
                    requested,
                    known=built_in,
                    built_in=built_in,
                )
                next_revision = self._next_provider_order_revision(current_revision)
                self._prefs["provider_order"] = order
                self._prefs["provider_order_revision"] = next_revision
                self._prefs["provider_order_applied_requests"] = (
                    record_applied_request(applied, request_id, next_revision)
                )
                self._save_prefs()
                return {
                    "ok": True,
    ...
    def get_providers(self) -> list[dict[str, Any]]:
        ...
        """
        out: list[dict[str, Any]] = []
        order = self.get_provider_order()["providers"]
        descriptors = {descriptor.name: descriptor for descriptor in provider_descriptors()}
        for name in order:
            d = descriptors[name]
    ...
    def pick_native_folder(self) -> dict[str, Any]:
        ...
        if sys.platform == "darwin":
            ...
            cmd = [
                ...
                "-e",
                'POSIX path of (choose folder with prompt "Give OpenLoop access to a folder"'
                + default_loc
                + ")",
            ]
        elif sys.platform == "win32":
            ...
            ps = (
                ...
                "$f.Description = 'Give OpenLoop access to a folder'; "
    ...
    def set_provider(
        ...
        with self._provider_config_lock:
            ...
            fields = fields or {}
            existing = self.secrets.get(f"provider:{name}") or {}
            was_configured = descriptor_configured(d, existing)
            ...
            first_connection = not was_configured and descriptor_configured(d, profile)
            secret_profile = f"provider:{name}"
            if first_connection:
                previous_raw = self.secrets.get_raw(secret_profile)
                previous_order, previous_revision = (
                    self._ensure_provider_order_under_config_lock()
                )
                target_order = promote_provider(previous_order, name)
                target_revision = self._next_provider_order_revision(
                    previous_revision
                )
                try:
                    self._write_provider_config_journal(
                        provider=name,
                    ...
                    self.secrets.put(secret_profile, profile)
                    with self._prefs_lock:
                        self._prefs["provider_order"] = target_order
                        self._prefs["provider_order_revision"] = target_revision
                        self._save_prefs()
                except OSError:
                    secret_compensated = False
                    prefs_compensated = False
                    try:
                        self.secrets.restore_raw(secret_profile, previous_raw)
                        secret_compensated = True
                    except OSError:
                        pass
                    try:
                        with self._prefs_lock:
                            self._prefs["provider_order"] = previous_order
                            self._prefs["provider_order_revision"] = previous_revision
                            self._save_prefs()
                        prefs_compensated = True
                    except OSError:
                        pass
                    if secret_compensated and prefs_compensated:
                        self._remove_provider_config_journal()
                    return {
                        "ok": False,
                        "error": "Could not save provider configuration.",
                    }
                self._remove_provider_config_journal()
            else:
                self.secrets.put(secret_profile, profile)
            self._refresh_provider(name)
            # Convenience: if the provider recommends a model and it's actually available, add it
            # to the curated list so it shows up in the composer right after configuring it.
            ...
            # First working provider wins the default: a default that already works is never stolen.
            if added and not self._provider_configured(self._model_provider(self.model)):
                self.set_default_model(added)
            order_state = self.get_provider_order()
            return {
                "ok": True,
                "provider": name,
                "recommended_model": rec,
                "provider_order": order_state["providers"],
                "provider_order_revision": order_state["revision"],
            }
    ...
    def _load_prefs(self) -> dict[str, Any]:
        try:
            ...
            return {}
        if "scratch_base" in prefs and "session_root" not in prefs:
            prefs["session_root"] = prefs.pop("scratch_base")
        return prefs
    ...
    def _save_prefs(self) -> None:
        with self._prefs_lock:
            write_private_text(
                self._prefs_path(),
                json.dumps(dict(self._prefs), indent=2),
            )

    def _provider_config_journal_path(self) -> Path:
        return self._data_base / "provider-config.pending.json"

    def _write_provider_config_journal(
        self,
        *,
        provider: str,
        ...
        write_private_text(
            self._provider_config_journal_path(),
            json.dumps(
                {
                    "provider": provider,
                    "previous_order": previous_order,
    ...
    def _remove_provider_config_journal(self) -> None:
        self._provider_config_journal_path().unlink(missing_ok=True)

    def _recover_provider_config_journal(self) -> None:
        with self._provider_config_lock:
            path = self._provider_config_journal_path()
            if not path.is_file():
                return
            try:
                journal = json.loads(path.read_text(encoding="utf-8"))
                provider = str(journal["provider"])
                descriptor = get_descriptor(provider)
                if descriptor is None:
                    raise ValueError("unknown provider")
                previous_order = validate_provider_ids(journal["previous_order"])
                target_order = validate_provider_ids(journal["target_order"])
                previous_revision = validate_revision(journal["previous_revision"])
                target_revision = validate_revision(journal["target_revision"])
            except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError):
                self._remove_provider_config_journal()
                return

            configured = descriptor_configured(
                descriptor,
                self.secrets.get(f"provider:{provider}") or {},
            )
            with self._prefs_lock:
                self._prefs["provider_order"] = (
                    target_order if configured else previous_order
                )
                self._prefs["provider_order_revision"] = (
                    target_revision if configured else previous_revision
                )
                self._save_prefs()
            self._remove_provider_config_journal()
    ...
    def get_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            "session_root": self._prefs.get("session_root")
            or self.DEFAULT_SESSION_ROOT,
            # Where new folders default to (folder picker / gate prefill) — falls back
            # to the session root so one setting covers both.
            "default_dir": self.default_dir(),
            # Real on-disk secrets location, so the UI shows the OS-native path instead of a
            # hardcoded POSIX one (Windows -> %APPDATA%\openloop, macOS/Linux -> ~/.config).
    ...
    def _nav_layout(self) -> str:
        """Sidebar layout: ``"flat"`` (default) or ``"grouped"``."""
        ...
        with self._prefs_lock:
            ...
            if cap_tokens is not None:
                try:
                    ...
                    return {
                        "ok": False,
                        "error": "compaction_cap_tokens must be a number",
                    }
    ...
    def validate_root(self, path: str) -> dict[str, Any]:
    ...
    def set_session_root(self, path: str) -> dict[str, Any]:
        """Set and persist the root where each OpenLoop scratch directory is
        created (default ~/OpenLoop). The raw value is stored so the UI shows it as entered;
        new conversations use it immediately (existing ones keep their provisioned dir).
        """
        path = (path or "").strip()
        if not path:
            return {"ok": False, "error": "empty path"}
        validation = self.validate_root(path)
        if not validation.get("ok"):
            return validation
        with self._prefs_lock:
            self._prefs["session_root"] = path
            self._save_prefs()
        return {"ok": True, **self.get_settings()}

    def set_scratch_base(self, path: str) -> dict[str, Any]:
        """Legacy REST/UI alias; persists the renamed session_root pref."""
        return self.set_session_root(path)
    ...
    def _has_manual_slack_inbox_binding(self) -> bool:
        for raw in self.inbox_routing.bindings():
            ...
            if raw.get("channel") == "slack" and "/" not in target:
                return True
        return False
    ...
    def set_inbox_binding(
        ...
        if channel == "slack":
            ...
            if "/" in target:
                return {"ok": False, "error": "Choose a destination channel."}
            if not self.secrets.get("slack:default"):
                return {
                    "ok": False,
    ...
    def _set_allowed(
        ...
        """Add or remove a sender on a local connector's allow-list."""
        user_id = str(user_id).strip()
        if not user_id:
            return {"ok": False, "error": "user_id required"}
        if team_id:
            return {"ok": False, "error": "workspace-scoped connectors are unsupported"}
        ...
        return {"ok": True, "allowed_users": sorted(allowed), "team_id": None}
    ...
    async def refresh_gateway(self) -> list[str]:
        ...
        print(f"[openloop] messaging gateway reloaded: {', '.join(started) or 'none'}")
        return started
    ...
    @staticmethod
    def _connector_write_request(request) -> bool:
        from ..permissions import RiskClass, classify

        metadata = getattr(request, "metadata", None)
        category = getattr(metadata, "category", "")
        return (
            category in {"connector", "messaging"}
            and classify(getattr(request, "tool_name", ""), metadata)
    ...
    def _scheduled_approver(self, task, session_id: str):
        ...
        async def approver(request):
            ...
            if request.tool_name in WRITE_TOOLS or (
                request.tool_name in name_allowed
                and not self._connector_write_request(request)
            ):
                return ApprovalOutcome.ONCE
    ...
    async def mirror_inbox_item(self, item) -> None:
        ...
        if not (binding.channel and self.gateway is not None):
            return
        if binding.channel == "slack":
            if "/" in binding.target or not self.slack_approval_owner_ids():
                return
    ...
    def _resolve_inbox_reply(self, event) -> bool:
        ...
        message carried an `[ol:<id>]` token (so it's consumed here, not routed as a new turn) —
    ...
    def running_state(self) -> dict[str, Any]:
        return {
            "running": bool(self._running_sessions),
            "count": len(self._running_sessions),
        }

    async def broadcast_running_state(self) -> None:
    ...
    async def _dispatch_inbound(self, event) -> None:
        ...
        if src.chat_type in ("channel", "group"):
            ...
            # the per-thread OpenLoop session.
    ...
    async def _route_mention(self, event, ms: MessageSource, subs) -> None:
        """@OpenLoop tagged in a channel. A subscribed OpenLoop session owns the channel
        and must answer; otherwise a per-thread session handles it — spawned on the
        ...
        if subs:
            # A subscribed OpenLoop session answers tags directly; no new session is spawned.
            msg = (
    ...
    async def _spawn_mention_session(
        ...
        """First tag in a thread: a new visible OpenLoop session that owns the thread. Its
        ...
        chan = f"#{src.chat_name}" if src.chat_name else src.chat_id
        sid = uuid.uuid4().hex
        engine = self.get_engine(sid, agent="openloop")
        if engine is None:
            self.unrouted.record(
    ...
    def save(self, session_id: str, engine: TurnEngine) -> None:
        ...
        if workspace and not project_id:
            try:
                ...
                if ws_path.parent == root_path and ws_path.name.endswith(session_id):
    ...
    # -- session roots (scratch + added folders) --------------------------------
            ...
            if self.session_store.load(session_id) is None:
                self.session_store.save(
                    SessionRecord(
                        ...
                        workspace_kind="managed",
                        managed_root=str(self.session_root().resolve()),
                        model=self.model,
    ...
    def delete_session(self, session_id: str) -> dict[str, Any]:
        ...
        # ...and its managed session dir. STRICTLY metadata-scoped: a real project folder the
        # user picked is never touched, even if it happens to live under the current session_root.
        if ok and record and record.workspace:
            ws = Path(record.workspace).expanduser()
            try:
                managed_root = (
                ...
                if (
                    record.workspace_kind == "managed"
                    and managed_root is not None
                    and ws.parent.resolve() == managed_root
                    and ws.name.endswith(session_id)
                    and ws.is_dir()
                ):
                    shutil.rmtree(ws)
            except OSError:
    ...
    def _scratch_workspace_error(self, workspace: Any) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            if ws.is_relative_to(self.session_root().resolve()):
                return {
                    "ok": False,
                    "error": (
