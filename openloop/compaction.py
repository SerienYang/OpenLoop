...
is never modified; only what is sent to the model. See docs/auto-compaction-spec.md.
...
SUMMARY_SYSTEM_PROMPT = """You are compacting an OpenLoop session history so OpenLoop can continue working in a smaller context. Write a structured summary of the conversation below. It is OpenLoop's ONLY memory of these turns, so preserve everything load-bearing.
...
Rules:
- Do NOT carry full file contents as truth. Note THAT a file was read/edited; OpenLoop re-reads if it needs the content again. Stale memory of a file is worse than no memory.
