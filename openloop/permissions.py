...
class PermissionEngine:
    ...
    # Background/unattended/connector-triggered runs must never auto-send external connector
    # writes. They still park an approval through the normal approver path.
    ...
    def evaluate(
        ...
        is_connector = category == "connector"
        risk = classify(tool_name, metadata, self.risk_overrides)
        is_connector_write = (
            risk is RiskClass.EXTERNAL and category in {"connector", "messaging"}
        )
        ...
        if self.force_connector_approval and is_connector_write:
            return Decision(
                False,
                "background connector write requires approval",
                needs_user=True,
            )
