"""Playwright-backed browser automation tools for OpenLoop.
...
class _BrowserController:
    def __init__(self) -> None:
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="openloop-browser"
        )
    ...
    def browser_screenshot(path: str = "") -> dict[str, Any]:
        def run(page):
            out = (
                ...
                if path
                else Path(tempfile.gettempdir()) / "openloop-browser-screenshot.png"
            )
            out = out.resolve()
