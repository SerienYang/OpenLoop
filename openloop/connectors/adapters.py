...
logger = logging.getLogger("openloop.connectors")
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        try:
            ...
            logger.warning(
                "python-telegram-bot not installed — `pip install openloop[messaging]`"
            )
            return False
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        try:
            ...
            logger.warning(
                "slack-bolt not installed — `pip install openloop[messaging]`"
            )
            return False
...
def make_adapter(platform: str, profile: dict) -> Optional[BasePlatformAdapter]:
    """Build a local inbound adapter from a connector profile."""
