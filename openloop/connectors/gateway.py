...
Lives inside the always-on `openloop-server` (started/stopped in its lifespan). On inbound:
...
class Gateway:
    def __init__(
        ...
        *,
        ...
        # [ol:<id>] token), it resolves the item and is consumed — not routed as a new turn.
    ...
    def _record_recent(self, event: MessageEvent) -> None:
        ...
        if not s.user_id:
            return
        # Keep the source scope in the key for adapters that provide one.
