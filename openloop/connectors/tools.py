...
_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "('#general' or 'general'; resolved against the connected workspace). Use this to "
...
def _parse_or_coerce(target: str) -> tuple[str, str, Optional[str]]:
    """parse_target, but a bare channel name ('all-openloop', '#general') coerces to
...
def _resolve_slack_channel(
    ...
    """Resolve a Slack channel name through the connected Socket Mode workspace."""
    ...
    result = list_channels(secrets, "default", query, limit=50)
    hits = [
        channel
        for channel in (result.get("channels") or [])
        if str(channel.get("name", "")).lower() == query.lower()
    ]
    if not hits:
        return None, (
            f"no Slack channel named #{query} in the connected workspace — "
            "check the name, or pass the full address (slack:C…)"
        )
    c = hits[0]
    chat_id = str(c["id"])
    if not c.get("is_member"):
        return None, (
            f"found #{query}, but the bot isn't a member — invite @OpenLoop to #{query} "
            "in Slack, then retry"
        )
    return chat_id, None
...
def _resolve_token(secrets: SecretStore, platform: str, chat_id: str) -> Optional[str]:
    """Pick the locally stored outbound bot token."""
    del chat_id
...
def make_send_message_tool(
    ...
    *,
    ...
    send_message.__openloop_schema__ = _SCHEMA
    return send_message
...
def make_send_file_tool(
    ...
    *,
    ...
    send_file.__openloop_schema__ = _FILE_SCHEMA
    return send_file
