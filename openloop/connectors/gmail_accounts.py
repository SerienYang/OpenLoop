...
`gmail:account:<email>` holds one mailbox's manually supplied OAuth token. Once
...
def connect_account(
    ...
    """Store one mailbox; the first connected account becomes the default."""
    migrate_legacy_default(secrets)
    email = _norm(profile.get("account"))
    if not email:
        return {"ok": False, "error": "google account id missing"}
