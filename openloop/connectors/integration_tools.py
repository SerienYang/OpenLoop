"""OpenLoop connector tools for first-party integrations.

These tools are intentionally local-first: credentials are read from the SecretStore at
execution time and never enter prompts.
"""
...
def _account_profile(
    ...
    """Resolve the requested or default profile for an account connector."""
...
def _gmail_profile(
    ...
    """Resolve the requested or default Gmail mailbox."""
...
def _gcal_profile(
    ...
    """Resolve the requested or default Google Calendar account."""
...
def _hubspot_profile(
    ...
    """Resolve the requested or default HubSpot portal and private-app token."""
...
def _github_auth(
    ...
    """Return GitHub headers for the locally stored personal access token."""
    del install, force
...
def _github_git_auth_args(secrets: SecretStore, owner: str) -> list[str]:
    ...
    line only and never lands in .git/config or a credential store."""
    import base64
...
def _github_call(
    ...
    """Call the GitHub API with the locally stored personal access token."""
    ...
    *,
    ...
    def browser_read_url(url: str, max_chars: int = 20000) -> dict[str, Any]:
        ...
        out = _request(
            "GET",
            url,
            headers={"User-Agent": "openloop/0.1 (+connector)"},
