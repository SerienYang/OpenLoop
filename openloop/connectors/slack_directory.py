...
hint "invite @OpenLoop in Slack" instead of silently failing to listen.
"""
...
def _bot_token(secrets: SecretStore, team_id: str) -> str:
    """The manually connected Socket Mode workspace's bot token."""
    del team_id
...
def list_channels(
    ...
    *,
    ...
    is a member. `is_member` lets the GUI hint "invite @OpenLoop" for the rest."""
    token = _bot_token(secrets, team_id)
    if not token:
        return {"ok": False, "error": "workspace not connected"}
