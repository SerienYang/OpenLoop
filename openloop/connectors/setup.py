...
def connector_list(secrets: SecretStore) -> list[dict[str, Any]]:
    ...
    for d in list_descriptors():
        ...
        entry = {
            ...
            # MCP-backed one-click uses vendor-hosted MCP + local OAuth.
            "mcp": bool(d.mcp_url),
            ...
            # Manual Socket Mode: humans allowed to resolve consequential prompts.
...
def connect_connector(
    ...
    *,
    ...
    if name == "gmail":
        ...
        profile["account"] = identity or "manual"
        return gmail_accounts.connect_account(secrets, profile)
    if name == "google_calendar":
        from . import gcal_accounts

        profile["account"] = identity or "manual"
        return gcal_accounts.connect_account(secrets, profile)
    if name == "hubspot":
        from . import hubspot_portals

        hub_id = "".join(ch for ch in str(identity or "") if ch.isdigit()) or "manual"
        profile["hub_id"] = hub_id
        profile["account"] = identity or f"portal {hub_id}"
        return hubspot_portals.connect_portal(secrets, profile)
    if d.account_field:
