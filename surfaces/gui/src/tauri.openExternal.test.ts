...
import { openExternal, setTrayLanguage } from "./tauri";
...
it("passes the interface language to the native tray", async () => {
  const invoke = vi.fn().mockResolvedValue(true);
  vi.stubGlobal("__TAURI__", { core: { invoke } });

  await setTrayLanguage("zh-CN");

  expect(invoke).toHaveBeenCalledWith("set_tray_language", { lang: "zh-CN" });
});
