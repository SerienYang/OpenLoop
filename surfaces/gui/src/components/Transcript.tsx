...
function approvalChip(
  ...
  return (
    <span
      ...
      title={resolved ? t("approved · {{mode}}", { mode: resolved.replace(/_/g, " ") }) : t("approved")}
    >
      ✓ {t("approved")}
    </span>
  );
...
function LineText({ line }: { line: HumanLine }) {
  ...
  return (
    <span className="min-w-0 text-[13px] leading-relaxed">
      <span className="text-muted">{t(line.pre)}</span>
      {line.obj && <span className="text-ink">{line.obj}</span>}
      {line.post && <span className="text-muted">{t(line.post)}</span>}
    </span>
  );
}
    ...
    <div>
      <div className="group flex items-baseline gap-2 px-2 py-0.5 rounded-lg hover:bg-paper" data-testid="turn-step">
        ...
        {!!tool.hidden && (
          ...
          >
            {tool.hidden} {t("hidden")}
          </span>
        )}
        {failed && <span className="text-[11px] text-danger shrink-0">{t(tool.status)}</span>}
        {!running && (
          <button
...
function TurnGroup({
  ...
  const stepsLabel =
    nSteps === 1
      ? t("{{count}} step", { count: nSteps })
      : t("{{count}} steps", { count: nSteps });
