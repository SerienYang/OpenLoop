...
// Approval buttons: branded border for the primary, quiet Deny.
const BTN_ACCENT =
    ...
    >
      ...
      {item.kind === "approval" ? (
        <div className="flex items-center gap-2 mt-2.5 flex-wrap">
          ...
          >
            {item.data?.tool ? t(approvalActionLabels(item.data.tool).allow) : t("Approve")}
          </button>
          ...
          {item.data?.task_id && item.data?.standing_target && (
            <button
              ...
              title={t("Always allow against {{target}} for “{{title}}” — revoke any time on its Automations page", { target: item.data.standing_target, title: item.data.task_title || t("this automation") })}
          ...
          >
            {item.data?.tool ? t(approvalActionLabels(item.data.tool).deny) : t("Deny")}
          </button>
        </div>
      ) : item.kind === "question" ? (
        <>
        ...
        <div className="flex items-center gap-2 mt-2.5">
          ...
          >
            {item.data?.path ? t("Grant") : t("Grant (no folder)")}
          </button>
