...
const relTime = (
  epoch?: number | null,
  tr: (k: string, v?: Record<string, string | number>) => string = (k) => k,
): string | null => {
  ...
  if (hrs < 48) return tr("{{hrs}}h ago", { hrs });
  return tr("{{days}}d ago", { days: Math.floor(hrs / 24) });
};
...
// Settings ▸ Models reuses onboarding §39's ProviderGallery/ProviderForm so the two
    ...
    return (
      <div>
        <ProviderGallery
          ...
          onExpandedChange={setProvidersExpanded}
        />
    ...
    <div>
      ...
      {ps.sel === "openai" && settings.source === "env" && (
        <p className="text-[12px] text-muted mt-3 leading-relaxed">
          {t("A key is set via <code>OPENAI_API_KEY</code> in this server's environment. You can override it above; the stored key is used only when the environment variable is absent.")}
        </p>
      )}
  ...
  return (
    <div className="space-y-3">
      <p className="text-[12.5px] text-muted leading-relaxed">
        {t("Enabled servers' tools are permission-gated. Changes apply to new sessions —")}{" "}
        <button
...
function AddForm({
  ...
  const [text, setText] = useState(EXAMPLE_MCP_CONFIG);
  ...
  const save = async () => {
    ...
    const entries =
      map && typeof map === "object" && !Array.isArray(map) && !map.command && !map.url
        ? Object.entries(map)
        : name.trim()
          ? [[name.trim(), parsed]]
          : null;
    if (!entries || entries.length === 0) {
      onError(t("Enter a server name for this config."));
      return;
    }
  ...
  return (
    <div className="space-y-2">
      <label className="block space-y-1">
        <span className="text-[12.5px] text-muted">{t("Server name")}</span>
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="w-full text-[13px] px-3 py-2 rounded-lg border border-line bg-paper text-ink outline-none focus:border-accent"
          placeholder="filesystem"
        />
      </label>
      <label className="block space-y-1">
        <span className="text-[12.5px] text-muted">{t("Server JSON")}</span>
        <textarea
          value={text}
      ...
      <div className="text-[11.5px] text-faint">
        {t("Paste one server config, or a full name → config map.")}
      </div>
      <div className="flex items-center gap-3">
  ...
  return (
    ...
    >
      <div className={SEC_H + " mb-2"}>
        {t("Messages from senders you haven't allowed · {{count}}", { count: items.length })}
      </div>
      <div className="space-y-2">
        {items.map((m) => (
          <div key={m.id} className="rounded-xl border border-line bg-paper p-2.5">
            <div className="flex items-center gap-2 text-[12px] text-muted">
              ...
              <span>{t("in {{chat}}", { chat: m.chat_name || m.chat_id })}</span>
...
export function ListeningSessionsBlock({ c }: { c: Connector }) {
  ...
  return (
    <div className="border-t border-line px-3.5 py-3" data-testid={`listening-${c.name}`}>
      <div className={SEC_H + " mb-2"}>{t("Sessions listening to {{title}} channels · {{count}}", { title: c.title, count: mine.length })}</div>
      {mine.length === 0 ? (
...
export function ConnectorTools({ c, onChanged }: { c: Connector; onChanged: () => void }) {
  ...
  return (
    <div className="border-t border-line px-3.5 py-3">
      <div className={SEC_H + " mb-2"}>{t("Tools exposed to OpenLoop")}</div>
      <div className="space-y-1.5">
        {c.tools.map((tool) => (
          <label
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block text-[11.5px] text-faint">
                {t("{{name}} · {{kind}} · asks approval", { name: tool.name, kind: tool.kind })}
              </span>
              <span className="block text-[11.5px] text-faint">
                {translateDynamic(
                  tool.description,
...
export function ConnectSetup({
  ...
  useEffect(() => {
    if (!waiting) return;
    const check = async () => {
      const connectors = await getConnectors().catch(() => []);
      if (connectors.find((connector) => connector.name === c.name)?.connected) {
        onConnected();
      }
    };
    check();
  ...
  return (
    <div className="border-t border-line px-3.5 py-3 space-y-3">
      {c.mcp && (
        <div className="space-y-2" data-testid="mcp-connect">
          <button
            ...
            disabled={waiting}
          >
            {waiting ? t("Check your browser…") : t("Connect {{title}} with one click", { title: c.title })}
          </button>
          {c.fields.length > 0 && (
      ...
      {lang === "zh-CN" && c.instructions.length > 0 ? (
        <p className="text-[12.5px] text-muted leading-relaxed">
          {t("Follow the service provider's instructions to create credentials, then fill in the fields below.")}
        </p>
        ...
        <label className="conn-field" key={f.key}>
          <span className="conn-field-label">
            {translateDynamic(
              f.label,
              lang,
              t("Credential field: {{key}}", { key: f.key }),
            )}
          ...
          {f.help && (
            <span className="conn-field-help">
              {translateDynamic(
                f.help,
                lang,
                t("Use the value provided by {{title}}.", { title: c.title }),
              )}
            </span>
          )}
        </label>
      ...
      {error && (
        <div className="text-[12.5px] text-danger">
          {translateDynamic(
            error,
            lang,
            t("Could not connect. Check the credentials and network, then try again."),
          )}
        </div>
      )}
    </div>
