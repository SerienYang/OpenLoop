...
const box = () => screen.getByPlaceholderText(/Ask OpenLoop/);

afterEach(() => {
  cleanup();
  vi.unstubAllGlobals();
  localStorage.removeItem("openloop:lang");
});

describe("Composer launch placement", () => {
  it("adds the launch wrapper class without changing the Composer surface", () => {
    stubFetch();
    const { container } = render(<Composer {...props({ placement: "launch" })} />);
    expect(
      container.querySelector(".composer-wrap")?.classList.contains("composer-wrap-launch"),
    ).toBe(true);
    expect(container.querySelectorAll(".composer")).toHaveLength(1);
  });

  it("translates the current permission mode in Chinese", () => {
    localStorage.setItem("openloop:lang", "zh-CN");
    stubFetch();
    render(
      <I18nProvider>
        <Composer {...props({ workspace: "" })} />
      </I18nProvider>,
    );
    const mode = screen.getByRole("button", { name: "模式" });
    expect(mode.textContent).toContain("执行前询问");
