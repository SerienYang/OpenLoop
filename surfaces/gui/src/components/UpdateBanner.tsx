...
// user mid-session would kill their running OpenLoop turn, and quiet self-mutation
...
export function UpdateBanner() {
  ...
  return (
    ...
    // 12px side margins. With no footer, the card sits directly above the bottom edge.
    <div
      className="fixed bottom-3 left-3 z-[35] w-[240px] rounded-xl border border-line bg-panel shadow-2xl px-4 py-3.5"
      role="status"
      data-testid="update-banner"
    >
      <div className="text-[13px] font-semibold">{t("Update available")}</div>
      <div className="text-[12px] text-muted mt-0.5">
        {t("OpenLoop v{{version}} is ready to install.", { version: update.version })}
      </div>
      {phase === "error" && (
