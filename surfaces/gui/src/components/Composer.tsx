...
interface Props {
  ...
  // When set, the Mode menu is shown. The folder/roots + branch controls left the
...
export function Composer(props: Props) {
  ...
  useEffect(() => {
    ...
    refresh();
    window.addEventListener("openloop:voice-input-changed", refresh);
    return () => window.removeEventListener("openloop:voice-input-changed", refresh);
  }, []);
    ...
    for (const a of read) {
      if (a.kind === "pdf" && a.data_url) {
        ...
        if (info?.ok && (info.pages ?? 0) > maxPages) {
          showAttachNotice(
            t("{{name}} skipped — {{pages}} pages is over your {{maxPages}}-page limit (Settings → Token savings)", {
              name: a.name,
              pages: info.pages ?? 0,
              maxPages,
            }),
        ...
        if (info && !info.ok) {
          showAttachNotice(
            t("{{name}} skipped — {{reason}}", { name: a.name, reason: info.error || t("could not read PDF") }),
          );
          continue;
        }
      }
  ...
  const placement = props.placement ?? "dock";

  return (
    <div
      className={
        placement === "launch"
          ? "composer-wrap composer-wrap-launch"
          : "composer-wrap px-6 pb-5 pt-4"
      }
    >
      {props.approvalSlot}
      ...
      <span className="sr-only" role="status" aria-live="polite">
        {dictation?.recording
          ? t("Listening, {{time}}", { time: recordingTime })
          : dictationBusy || ""}
      </span>
    </div>
  );
  ...
  return (
    <div className="relative">
      <button
        ...
        title={
          showBar
            ? t("Context window {{pct}}% full · {{tokens}} tokens this session", {
                pct,
                tokens: formatTokens(total),
              })
            : t("Token usage this session: {{tokens}}", { tokens: formatTokens(total) })
        }
        ...
        <>
          ...
          >
            {contextWindow ? (
              <div className="mb-2.5">
                ...
                <div className="mt-1 text-[11.5px] text-muted tabular-nums">
                  {formatTokens(usage.context)} {t("of")} {formatTokens(contextWindow)} · {pct}%
                </div>
              </div>
            ) : usage.context > 0 ? (
            ...
            <div className="mt-2 pt-2 border-t border-line flex items-baseline justify-between text-[11.5px]">
              ...
              <span className="text-ink tabular-nums">{formatTokens(total)} {t("tokens")}</span>
            </div>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        aria-label={t("Mode")}
        title={
          t("Mode: {{label}}", { label: t(current?.label || mode) }) +
