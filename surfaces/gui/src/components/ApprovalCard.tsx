...
export function scopeNote(
  ...
  ) => translate?.(key, vars) ?? key.replace(/\{\{(\w+)\}\}/g, (_, k) => String(vars?.[k] ?? ""));
  ...
  if (EXTERNAL.has(name)) {
    ...
    return {
      text: tr("leaves this computer → {{target}}", {
        target: names[platform] || platform || tr("a connected chat"),
      }),
      external: true,
    };
  }
  ...
  return {
    text: overwrite
      ? tr("stays on this computer · overwrites the existing file")
      : tr("stays on this computer"),
    external: false,
  };
}
      ...
      {!connector && !offerStanding && item.name !== "run_shell" && item.name !== "save_skill" && (
        <button
          ...
          title={t("Always allow {{action}} for this session", {
            action: TOOL_VERBS[item.name]?.toLowerCase() || item.name,
          })}
  ...
  return (
    <div className={"approval" + (scope.external ? " approval-external" : "") + dock}>
      <div className="approval-top">
        ...
        <span className={"approval-scope" + (scope.external ? " out" : "")}>{t(scope.text)}</span>
      </div>
          ...
          {grants.map((g, i) => (
            <div className="approval-grant" key={i} data-access={g.access}>
              ...
              <span className="grant-line">
                {t(TOOL_VERBS[g.tool] || g.tool)} <code className="approval-tool">{g.target}</code>
      ...
      {item.resolved ? (
        <div className="resolved">{t("Approved:")} {t(item.resolved.replace("_", " "))}</div>
      ) : (
        <Buttons
          item={item}
          onApprove={onApprove}
