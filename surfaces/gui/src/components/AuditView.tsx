...
import { clearAudit, getAudit, type AuditEvent } from "../api";
...
export function AuditLogPanel() {
  ...
  const clear = async () => {
    await clearAudit().catch(() => undefined);
    setEvents([]);
  };
  ...
  return (
    <section>
      <PanelHead
        ...
        sub={t("Recent connector and browser tool activity. Entries are kept for 30 days.")}
      />
          ...
          <div className="flex items-center gap-2 flex-wrap mb-4">
            ...
            <button className="text-[12.5px] px-3 py-1.5 rounded-lg border border-line bg-paper hover:border-lineStrong" onClick={clear}>
              {t("Clear log")}
            </button>
          </div>
