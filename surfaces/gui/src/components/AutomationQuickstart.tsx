...
// picking a card expands the configure card below the grid: connection status,
// channel-by-name, day × time, and the §25 consent line for write recipes.
...
export function AutomationQuickstart({
  ...
  // Connector state drives the card dots; poll while a template is configured.
  ...
  const startConnect = (name: string) => setPendingConn(name);
        ...
        >
          ...
          {pendingConn && (
            <div
              ...
              data-testid="ob-local-connect-pane"
            >
              <span className="block text-[13px] text-ink font-medium">
                {t("Configure {{name}} from Connectors", {
                  name: connState(pendingConn)?.title || pendingConn,
                })}
              </span>
              {t(
                "Open Settings, choose Connectors, and add the credential there. This template will unlock automatically.",
              )}
              <button
                className="block mt-2 text-[12px] text-accent hover:underline"
              ...
              {picked.needsChannel && (
                <>
                  ...
                  <p className="text-[11px] text-warnInk mt-1">
                    {t("The bot must be a member of the channel — invite @OpenLoop in Slack if it isn't.")}
                  </p>
                </>
              )}
