    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead title={tr("Automations")} sub={tr("Recurring tasks OpenLoop runs on a schedule.")} />
        </div>
        <button
      ...
      <div className="text-[12px] text-faint flex gap-1.5 mb-4">
        ...
        <span>
          {tr("Runs only while openloop-server is up — a missed schedule catches up once when it next starts.")}
        </span>
      </div>
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            {tr("No scheduled tasks yet — use a template above, click")} <strong>+ {tr("New automation")}</strong>
            {tr(", or just ask OpenLoop in a session.")}
          </div>
        )
      ) : (
            ...
            >
              <div className="flex items-center justify-between gap-2.5 mb-1">
                ...
                <button
                  ...
                  title={tr("Delete automation")}
                  aria-label={tr("Delete {{title}}", { title: t.title })}
                  onClick={async (e) => {
                    e.stopPropagation();
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {t.enabled ? t.schedule : tr("Paused")} · {tr("next")} {fmt(t.next_run)} · {t.run_count === 1 ? tr("{{count}} run", { count: t.run_count }) : tr("{{count}} runs", { count: t.run_count })}
                {t.last_status ? ` · ${tr("last")} ${t.last_status}` : ""}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="sched-detail">
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled ? t("Active · next {{time}}", { time: fmt(task.next_run) }) : t("Paused")} · {task.schedule}
          </div>
        )}
          ...
          >
            <div className="sched-run-row">
              <span>
                ...
                {r.artifacts.length > 0 && <span className="dim"> · {r.artifacts.length} {r.artifacts.length === 1 ? t("file") : t("files")}</span>}
              </span>
