...
// Default/Sandbox tags) + Access & privacy
...
// the header button and a private-app token form.
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {t("Connect a HubSpot private app. Its scopes control read and write access.")}
          </div>
        </div>
      )}
  ...
  return (
    <div className={ROW} data-testid={`hubspot-portal-${p.hub_id}`}>
      <span className="min-w-0 flex-1 flex items-center gap-2">
        ...
        <span className={TAG_QUIET}>{t("private app")}</span>
      </span>
      {!p.default && (
        <button
