...
export const platformOS = (): string => {
  const injected = (globalThis as any).__OPENLOOP_PLATFORM__;
...
export type AwakeRule = "off" | "while_running" | "always";
...
/** Sleep-prevention rule. "always" preserves the old keep-awake behavior; "while_running"
 * holds the system awake only while the sidecar reports active work. */
export const getAwakeRule = () => invoke<AwakeRule>("get_awake_rule");
export const setAwakeRule = (rule: AwakeRule) => invoke<AwakeRule>("set_awake_rule", { rule });
export const setAwakeRunning = (running: boolean) => invoke<boolean>("set_awake_running", { running });

/** Keep the native tray menu in sync with the interface language. */
export const setTrayLanguage = (lang: "en" | "zh-CN") =>
  invoke<boolean>("set_tray_language", { lang });
...
export function openExternal(url: string): void {
  ...
  if (opener?.openUrl) {
    console.log("[openloop] openExternal -> opener.openUrl", url);
    opener.openUrl(url).catch((err: unknown) => {
      console.error("[openloop] opener.openUrl failed, falling back to window.open:", err);
      window.open(url, "_blank", "noopener,noreferrer");
    });
    return;
  }
  console.warn("[openloop] __TAURI__.opener missing, window.open fallback:", url);
  window.open(url, "_blank", "noopener,noreferrer");
}
