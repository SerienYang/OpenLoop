...
// __OPENLOOP_PLATFORM__; this simulates each platform and checks the overlay class.
test("windows platform gets no tauri-overlay layout", async ({ page }) => {
  await page.addInitScript(() => {
    (window as any).__TAURI__ = {}; // simulate the desktop shell
    (window as any).__OPENLOOP_PLATFORM__ = "windows";
  });
  await page.goto("/");
...
test("macos platform keeps the overlay layout", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    (window as any).__OPENLOOP_PLATFORM__ = "macos";
  });
  await page.goto("/");
