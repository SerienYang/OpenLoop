// Session-screen cleanup: contextual top-left controls and the centered model facts.
...
test("facts subtitle: absent on a fresh session, model-only after the first turn, inert", async ({
  ...
  // Fresh-ish (boot-resumed, no rendered history): no subtitle; model selection stays live.
  ...
  // The subtitle is a plain model fact, not an interactive control.
