// Cold-boot fixes (owner-hit 2026-07-23): the splash wears the real OpenLoop mark
// (the app SVG, not the ✦ text glyph that read as another product's logo), and the
...
test("boot splash shows the OpenLoop mark, not the sparkle glyph", async ({ page }) => {
  ...
  await expect(page.getByText(/Starting OpenLoop|Restoring your session/)).toBeVisible();
});
