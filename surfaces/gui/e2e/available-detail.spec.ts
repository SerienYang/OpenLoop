...
test("Chinese connector surfaces replace unknown API copy with localized context", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("connector-telegram")).toContainText(
    "连接并使用 Telegram。",
  );
  await page.getByTestId("connector-gmail").click();
  const detail = page.getByTestId("available-detail");
  await expect(detail).toContainText(
    "连接 Gmail 后，可在 OpenLoop 中使用该服务支持的内容与操作。",
  );
  await expect(detail).not.toContainText(
    "Search, summarize, and send over your Gmail.",
  );
  await page.getByTestId("available-tools-toggle").click();
  await expect(detail).not.toContainText("Search messages.");
