...
test("Settings toggle turns the context bar on; default is the session total", async ({ page }) => {
  ...
  await page.getByText("Draft the launch note").first().click();
  const box = page.getByPlaceholder(/Ask OpenLoop|Describe the outcome/);
  await box.fill("hello before bar");
  await page.getByRole("button", { name: "Send" }).click();
  await expect(page.getByText("Echo: hello before bar", { exact: false }).first()).toBeVisible({
    timeout: 10_000,
  });
  const chip = page.getByTestId("usage-chip");
  ...
  await page.goto("/");
  await page.getByText("Draft the launch note").first().click();
  await page.getByPlaceholder(/Ask OpenLoop|Describe the outcome/).fill("hello with bar");
  await page.getByRole("button", { name: "Send" }).click();
  await expect(page.getByText("Echo: hello with bar", { exact: false }).first()).toBeVisible({
    timeout: 10_000,
  });
