...
function sidecarToken(): string {
  const state =
    process.env.OPENLOOP_STATE_DIR ||
    (process.platform === "win32"
      ? join(process.env.APPDATA || homedir(), "openloop")
      : join(homedir(), ".config", "openloop"));
  try {
...
export function backendFetch(path: string, init: RequestInit = {}): Promise<Response> {
  ...
  if (token) headers.set("X-OpenLoop-Token", token);
...
export async function scratchBaseIfReady(): Promise<string | null> {
  try {
    ...
    if (res.ok && s.model_ready) {
      return String(s.scratch_base || "~/OpenLoop").replace(/^~(?=\/|$)/, homedir());
    }
  } catch {
    /* backend unreachable */
  }
...
/** Open a fresh OpenLoop session via the split button's persona menu. */
export async function startOpenLoopSession(page: Page) {
  await page.goto("/");
...
export async function sendTask(page: Page, text: string) {
  await page.getByPlaceholder(/Ask the openloop/).fill(text);
