...
import { newestFile, scratchBaseIfReady, selectMode, sendTask, startOpenLoopSession } from "./helpers";
...
test("live: agent writes fib.md to its scratch workspace, verified on disk", async ({ page }) => {
  ...
  test.skip(!scratchBase, "live backend not ready — start openloop-server and configure a model");

  await startOpenLoopSession(page);
