...
#   3. `tauri build --bundles app` → OpenLoop.app (resources are copied in).
...
#   - A frozen Python environment at .venv (repo root):
#       uv sync --frozen --extra messaging --extra browser --extra bedrock --extra release
#     The release extra supplies PyInstaller/typer/tzdata; aisuite stays git-pinned in uv.lock.
#
...
# NOTARYTOOL_API_KEY_ID / NOTARYTOOL_API_ISSUER_ID — exported, or in $OPENLOOP_NOTARY_ENV, or in
# `.openloop-notary.env` one directory ABOVE the repo (shared by every clone/worktree on a machine,
...
# OPENLOOP_SKIP_NOTARIZE=1 to sign but skip the slow notary round-trip. Neither is distributable.
#
...
#   OPENLOOP_EXPERIMENTAL=1 ./build_dmg.sh
set -euo pipefail
...
if [ -n "${APPLE_CERTIFICATE:-}" ] && [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  KC_DIR="$(mktemp -d)"
  KC="$KC_DIR/openloop-signing.keychain-db"
...
echo "==> [1/5] PyInstaller: bundling openloop-server ($TRIPLE)"
"$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/openloop-server.spec"
"$PLATFORM/.venv/bin/python" "$HERE/sanitize_brand_residue.py" "$HERE/dist/openloop-server"
...
cp -RL "$HERE/dist/openloop-server" "$GUI/src-tauri/binaries/sidecar"
...
"$PLATFORM/.venv/bin/python" "$HERE/sanitize_brand_residue.py" "$GUI/src-tauri/binaries/sidecar"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "openloop-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/openloop-server"
fi
...
# `.openloop-updater.env` one directory above the repo (same convention as the notary env).
...
# Clean any prior bundle first: if `tauri build` fails or short-circuits, a stale local app can
# survive under target/release/bundle/macos and get wrapped into a fresh DMG. That exact failure
...
"$PLATFORM/.venv/bin/python" "$HERE/sanitize_brand_residue.py" "$GUI/src-tauri/target/release/sidecar"
...
[ -f "$APP_BIN" ] || { echo "ERROR: missing app executable: $APP_BIN" >&2; exit 1; }
# Refuse to package a bundle whose executable predates THIS build invocation — that means we are
# about to ship stale output from an earlier local run instead of the app this command just built.
APP_BIN_MTIME="$(stat -f %m "$APP_BIN" 2>/dev/null || stat -c %Y "$APP_BIN")"
...
if [ "${OPENLOOP_SKIP_NOTARIZE:-}" = "1" ] && [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  echo "==> [5/5] OPENLOOP_SKIP_NOTARIZE=1 — signing container, SKIPPING notarize/staple (do not distribute)"
  ...
  NOTARY_ENV="${OPENLOOP_NOTARY_ENV:-$PLATFORM/../.openloop-notary.env}"
  ...
  if [ -n "${NOTARYTOOL_API_KEY_PATH:-}" ] && [ -n "${NOTARYTOOL_API_KEY_ID:-}" ] \
    ...
    echo "    (env, \$OPENLOOP_NOTARY_ENV, or $NOTARY_ENV)."
  fi
else
