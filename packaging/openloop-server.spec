...
"""PyInstaller spec for the bundled `openloop-server` (desktop sidecar).
...
archive to a temp dir on EVERY launch — 6-7s of "Starting OpenLoop…" splash (measured; the
actual Python import is ~0.5s). The wrinkles handled here:
  - aisuite is a regular pip dependency (git-pinned in pyproject.toml); collect the internal package +
...
# is stripped, not just disabled. Self-builders opt in with OPENLOOP_EXPERIMENTAL=1; the
# connector descriptor loader treats the missing package as a no-op.
INCLUDE_EXPERIMENTAL = os.environ.get("OPENLOOP_EXPERIMENTAL") == "1"

hiddenimports = []
datas = []
binaries = []

for pkg in ("openloop", "aisuite", "mcp", "ddgs", "croniter", "docstring_parser"):
    hiddenimports += collect_submodules(pkg)

if not INCLUDE_EXPERIMENTAL:
    hiddenimports = [
        m for m in hiddenimports if not m.startswith("openloop.connectors.experimental")
    ]

# `websockets` powers Uvicorn's local session/event WebSocket server. Collect it
# explicitly so the packaged sidecar keeps its WebSocket transport.
...
a = Analysis(
    ...
    + ([] if INCLUDE_EXPERIMENTAL else ["openloop.connectors.experimental"]),
    noarchive=False,
)
pyz = PYZ(a.pure)
exe = EXE(
...
# Onedir: dist/openloop-server/{openloop-server[.exe], _internal/}. The build scripts stage
